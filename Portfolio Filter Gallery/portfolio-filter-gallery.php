...
if (!class_exists('Awl_Portfolio_Filter_Gallery')) {
	...
	class Awl_Portfolio_Filter_Gallery {
		...
		public function set_filter_gallery_shortcode_column_name($defaults)
		{
			...
			foreach ($defaults as $key => $value) {
				if ($key == 'date') {  // When we find the date column
					$new['_filter_gallery_shortcode'] = __('短代碼', 'portfolio-filter-gallery');  // Add the shortcode column before it
				}
				$new[$key] = $value;
			}
		...
		public function pfg_menu() {
			$filter_menu = add_submenu_page('edit.php?post_type=' . PFG_PLUGIN_SLUG, __('篩選集', 'portfolio-filter-gallery'), __('篩選集', 'portfolio-filter-gallery'), 'manage_options', 'pfg-filter-page', array($this, 'awl_filter_page'));
			$doc_menu = add_submenu_page('edit.php?post_type=' . PFG_PLUGIN_SLUG, __('文件檔案', 'portfolio-filter-gallery'), __('文件檔案', 'portfolio-filter-gallery'), 'manage_options', 'sr-doc-page', array($this, 'pfg_doc_page'));
			$upgrade_menu = add_submenu_page('edit.php?post_type=' . PFG_PLUGIN_SLUG, __('購買進階功能', 'portfolio-filter-gallery'), __('購買進階功能', 'portfolio-filter-gallery'), 'manage_options', 'https://awplife.com/wordpress-plugins/portfolio-filter-gallery-wordpress-plugin/');
		...
		public function admin_add_meta_box() {
			add_meta_box(__('Add Portfolio Filter Gallery', 'portfolio-filter-gallery'), __('新增作品集篩選畫廊', 'portfolio-filter-gallery'), array(&$this, 'pfg_image_upload'), 'awl_filter_gallery', 'normal', 'default');
			add_meta_box(__('pfg-shortcode', 'portfolio-filter-gallery'), __('複製短代碼', 'portfolio-filter-gallery'), array(&$this, 'PFG_Shortcode'), 'awl_filter_gallery', 'side', 'default');
			...
			add_meta_box(__('pfg-youtube', 'portfolio-filter-gallery'), __('篩選器 拖曳&丟棄', 'portfolio-filter-gallery'), array(&$this, 'PFG_Youtube'), 'awl_filter_gallery', 'side', 'default');
